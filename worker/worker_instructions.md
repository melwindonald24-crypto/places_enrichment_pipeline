...
6. Return exactly one response accepted by `validate_batch_response()`: protocol `1`, same `batch_id`, same `artifact_blob_sha`, one non-null result per request job, exact order, no extra fields.

7. Run `validate()` on every result and `validate_batch_response()` on the complete response.
